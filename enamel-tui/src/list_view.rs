...
use cursive::event::{AnyCb, Callback, Event, EventResult, Key};
use cursive::{Rect, Vec2};
use cursive::view::{IntoBoxedView, Selector, View};
use cursive::Cursive;
use cursive::Printer;
use cursive::With;
use log::debug;
...
impl View for ListView {
        ...
        for (i, child) in self.children.iter().enumerate() {
            match child {
                ListChild::Row(ref label, ref view) => {
                    ...
                    view.draw(
                        &printer.offset((0, i)).focused(i == self.focus),
                    );
                }
                ListChild::Delimiter => (),
            }
    ...
    fn required_size(&mut self, req: Vec2) -> Vec2 {
        ...
        Vec2::new(view_size, self.children.len())
    }
    ...
    fn layout(&mut self, size: Vec2) {
        ...
        for child in self.children.iter_mut().filter_map(ListChild::view) {
            child.layout(Vec2::new(size.x, 1));
        }
    }

...
impl Config {
    pub fn load(location: &Path) -> Self {
        ...
        let expanded = shellexpand::full(location.to_str().unwrap())
            .unwrap()
            .into_owned();
...
pub struct MailDirConfig {
    #[serde(
        default = "default_maildir_synchronize_flags",
...
fn parse_csv<'de, D>(d: D) -> Result<Vec<String>, D::Error>
where
    D: Deserializer<'de>,
{
    serde::de::Deserialize::deserialize(d)
        .map(|x: Option<String>| x.unwrap().split(';').map(|s| s.to_string()).collect())
}
...
where
    D: Deserializer<'de>,
{
    serde::de::Deserialize::deserialize(d).map(|x: Option<String>| x.unwrap().parse().unwrap())
}
